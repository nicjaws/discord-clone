...
function createChannel(streamIn) {
  ...
  let handleIncomingPacket = (bytes) => {
    if (isFirstPacket) {
      ...
      if (binaryVersion !== "0.9.5") {
        throw new Error(`Cannot start service: Host version "${"0.9.5"}" does not match binary version ${JSON.stringify(binaryVersion)}`);
      }
      return;
    }
...
var ensureServiceIsRunning = () => {
  ...
  let child = child_process.spawn(command, args.concat(`--service=${"0.9.5"}`, "--ping"), {
    windowsHide: true,
...
var runServiceSync = (callback) => {
  ...
  callback(service);
  let stdout = child_process.execFileSync(command, args.concat(`--service=${"0.9.5"}`), {
    cwd: process.cwd(),
    windowsHide: true,
    input: stdin,
